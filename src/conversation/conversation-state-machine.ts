...
import { TranscriptClearedEvent, TranscriptDeltaEvent, TranscriptEvent, TranscriptFinalEvent, TranscriptionStatusEvent, TranscriptRedoEvent } from '../types/speech-to-text';
...
export type ConversationTerminationReason = 'manual-stop' | 'session-ended' | 'fatal-error' | 'shutdown';
export type InterruptSource = 'vad' | 'user-command' | 'system' | 'copilot';
export type SuspensionReason = 'session-renewal' | 'network-recovery' | 'diagnostics' | string;
...
export class ConversationStateMachine implements ServiceInitializable {
  ...
  async startConversation(context?: ConversationContext): Promise<void> {
    ...
    if (this.currentState !== 'idle' && this.currentState !== 'terminating') {
      this.logger.warn('Attempted to start conversation while already active', { state: this.currentState });
      return;
    }
  ...
  async endConversation(reason: ConversationTerminationReason = 'manual-stop'): Promise<void> {
  ...
  getState(): { state: ConversationState; previousState: ConversationState; metadata: StateMetadata; turnContext?: TurnContext } {
    return {
  ...
  async notifyTranscriptionStatus(event: TranscriptionStatusEvent): Promise<void> {
    ...
    switch (event.status) {
      case 'speech-started':
        ...
        this.transition('listening', 'stt.partial', { sessionId: event.sessionId });
        break;
      case 'speech-stopped':
        this.transition('processing', 'vad.end', { sessionId: event.sessionId });
        break;
      case 'thinking':
        this.transition('processing', 'stt.partial', { sessionId: event.sessionId });
        break;
      case 'paused':
        this.transition('suspended', 'system.suspend', { sessionId: event.sessionId, suspensionReason: 'stt-paused' });
        break;
      case 'error':
        this.transition('faulted', 'error', { sessionId: event.sessionId, reason: event.detail });
        break;
      default:
        break;
    }
  ...
  async notifyTts(event: TtsPlaybackEvent): Promise<void> {
    ...
    switch (event.type) {
      case 'speaking-state-changed': {
        const nextState = event.data?.state === 'speaking' ? 'speaking' : 'listening';
        const cause: TransitionCause = event.data?.state === 'speaking' ? 'tts.bufferReady' : 'tts.complete';
        if (event.data?.state === 'speaking') {
          this.beginAssistantTurn(new Date(event.timestamp).toISOString());
        }
        this.transition(nextState, cause, { sessionId: this.sessionInfo?.sessionId });
        break;
      }
      case 'playback-complete':
        this.completeAssistantTurn(new Date(event.timestamp).toISOString());
        this.transition('listening', 'tts.complete', { sessionId: this.sessionInfo?.sessionId });
        break;
      case 'interrupted':
      ...
      case 'playback-error':
        this.transition('faulted', 'error', {
          ...
          pendingActions: event.data?.error ? [event.data.error.code] : undefined
        });
        break;
      default:
        break;
  ...
  handleUserInterrupt(source: InterruptSource, message?: string): void {
    ...
    if (this.currentState === 'speaking' && this.currentTurn && this.currentTurn.turnRole === 'assistant') {
  ...
  resume(): void {
    ...
    const targetState = snapshot.state === 'suspended' ? 'listening' : snapshot.state;
  ...
  ingestInterruptionEvent(event: InterruptionTurnEvent): void {
    ...
    switch (event.type) {
      case 'interruption':
        this.handleUserInterrupt('vad', 'Interruption engine reported barge-in');
        break;
      case 'turn-started':
        if (event.turn?.role === 'assistant') {
          this.beginAssistantTurn(event.timestamp);
          this.transition('speaking', 'tts.bufferReady', { sessionId: this.sessionInfo?.sessionId });
        } else if (event.turn?.role === 'user') {
          this.beginUserTurn(event.timestamp);
          this.transition('listening', 'stt.partial', { sessionId: this.sessionInfo?.sessionId });
        }
        break;
      case 'turn-ended':
        if (event.turn?.role === 'user') {
          this.transition('processing', 'stt.final', { sessionId: this.sessionInfo?.sessionId });
  ...
  private handleTranscriptDelta(event: TranscriptDeltaEvent): void {
    ...
    this.transition('listening', 'stt.partial', { sessionId: event.sessionId, transcriptId: event.utteranceId });
    this.appendTranscriptContent(event.delta, event.confidence, event.metadata.serverVad?.state === 'start');
  }
  ...
  private beginAssistantTurn(timestamp?: string): void {
    if (this.currentTurn?.turnRole === 'assistant' && !this.currentTurnEnded()) {
      return;
    }
  ...
  private appendTranscriptContent(content: string, confidence?: number, markActive = false): void {
    if (!this.currentTurn) {
      return;
    }
  ...
  private appendAssistantTranscript(content: string, event: CopilotResponseEvent): void {
    if (!content) {
      return;
    }
    if (!this.currentTurn || this.currentTurn.turnRole !== 'assistant') {
  ...
  private transition(target: ConversationState, cause: TransitionCause, metadata?: StateMetadata): void {
    ...
    if (target === 'faulted') {
      ...
      if (this.isCircuitOpen()) {
        this.logger.warn('Conversation circuit breaker open; remaining in faulted state', { until: new Date(this.circuitOpenUntil).toISOString() });
    ...
    this.logger.debug('Conversation state transition', { from, to: target, cause, metadata: event.metadata });
  }
    ...
    for (const listener of listeners) {
      try {
        ...
        if (result instanceof Promise) {
          void result.catch(error => this.logger.error('State change handler failed', { error: error?.message ?? error }));
        }
      } catch (error: any) {
        this.logger.error('State change listener threw', { error: error?.message ?? error });
      }
    }
  }
  ...
  private emitTurnEvent(type: TurnEvent['type'], context: TurnContext): void {
    ...
    for (const listener of listeners) {
      try {
        ...
        if (result instanceof Promise) {
          void result.catch(error => this.logger.error('Turn event handler failed', { error: error?.message ?? error }));
        }
      } catch (error: any) {
        this.logger.error('Turn listener threw', { error: error?.message ?? error });
      }
    }
  }
  ...
  private registerFault(timestampIso: string): void {
    ...
    this.faultTimestamps = this.faultTimestamps.filter(ts => timestamp - ts <= CIRCUIT_BREAKER_WINDOW_MS);
    if (this.faultTimestamps.length >= CIRCUIT_BREAKER_THRESHOLD) {
      this.circuitOpenUntil = Date.now() + CIRCUIT_BREAKER_COOLDOWN_MS;
      this.logger.warn('Conversation fault threshold reached; circuit breaker engaged', { until: new Date(this.circuitOpenUntil).toISOString() });
    }
  }
  ...
  private ensureInitialized(operation: string): void {
    if (!this.initialized) {
      throw new Error(`ConversationStateMachine must be initialized before ${operation}`);
    }
  }
}
