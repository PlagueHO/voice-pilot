...
export class InterruptionEngineImpl implements InterruptionEngine {
  ...
  constructor(options?: {
    logger?: Logger;
    hooks?: InterruptionEngineHooks;
    policy?: Partial<InterruptionPolicyConfig>;
  }) {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.debug("Interruption engine initialized", {
  ...
  private ensureReady(operation: string): void {
    if (!this.initialized) {
      throw new Error(
        `InterruptionEngine must be initialized before ${operation}`,
      );
    }
  }

  private validatePolicy(
    policy: InterruptionPolicyConfig,
  ): InterruptionPolicyConfig {
  ...
  private registerInterruption(now: number): number {
    ...
    while (
      this.interruptionHistory.length &&
      now - this.interruptionHistory[0] > INTERRUPTION_HISTORY_WINDOW_MS
    ) {
      this.interruptionHistory.shift();
    }
    if (this.interruptionHistory.length >= 3) {
      this.cooldownEndsAt =
        now + Math.max(COOLDOWN_DURATION_MS, this.policy.completionGraceMs);
  ...
  private onAssistantStopped(): void {
    this.transitionState(
      this.userSpeaking ? "listening" : "thinking",
  ...
  private async cancelAssistantPlayback(
    type: string,
    reason: string,
  ): Promise<void> {
    ...
    try {
      ...
      if (latency > this.policy.interruptionBudgetMs) {
        this.logger.warn("Assistant cancellation exceeded budget", {
          latency,
          budget: this.policy.interruptionBudgetMs,
        });
      }
    } catch (error: any) {
      this.logger.error("Failed to cancel assistant playback", {
  ...
  private async triggerAssistantResponse(): Promise<void> {
    ...
    try {
      ...
      this.logger.error("Failed to request assistant response", {
        error: error?.message ?? error,
      });
    }
  }
  ...
  private emitEvent(
    ...
    for (const listener of Array.from(this.listeners)) {
      try {
        ...
        this.logger.error("Interruption listener failed", {
          error: error?.message ?? error,
          type: event.type,
        });
      }
  ...
  private buildDiagnostics(
    overrides?: TurnEventDiagnostics,
  ): TurnEventDiagnostics {
  ...
  private cloneTurn(turn: TurnDescriptor | null): TurnDescriptor | undefined {
    ...
    return {
      ...turn,
      interruption: turn.interruption ? { ...turn.interruption } : undefined,
    };
  }
  ...
  private normalizeInterruptionSource(
    source: SpeechActivityEvent["source"],
  ): InterruptionInfo["source"] {
    if (source === "manual") {
      return "system";
    }
    return source;
