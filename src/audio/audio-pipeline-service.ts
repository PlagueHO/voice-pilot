...
import {
  ...
  RealtimeTurnEvent,
} from "./turn-detection-coordinator";
...
export class AudioPipelineService implements ServiceInitializable {
  ...
  constructor(config: AudioPipelineConfig, logger?: Logger) {
    ...
    this.turnDetectionConfig = normalizeTurnDetectionConfig(
      this.config.realtime.turnDetection,
    );
    this.turnDetectionCoordinator = new AzureTurnDetectionCoordinator(
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.logger.error("Failed to initialize audio pipeline", {
        error: error.message,
      });
      throw error;
    }
  ...
  private setupEventHandlers(): void {
    ...
    this.audioCapture.addEventListener(
      "captureStarted",
      (event: CaptureStartedEvent) => {
        this.sessionState.deviceId =
          event.data.settings.deviceId ?? event.data.trackId;
        this.emitStateChange();
      },
    );
    ...
    this.audioCapture.onAudioData(async (audioData: Buffer) => {
      try {
        ...
        this.logger.error("Failed to send audio data", {
          error: error.message,
        });
      }
    });
  ...
  private handleTurnDetectionEvent(event: RealtimeTurnEvent): void {
    try {
      ...
      this.logger.error("Turn detection coordinator failed to process event", {
  ...
  onSessionState(
    callback: (state: VoiceSessionState) => void,
  ): vscode.Disposable {
  ...
  private emitStateChange(): void {
    ...
    for (const listener of Array.from(this.sessionStateListeners)) {
      try {
        ...
        this.logger.error("Session state listener failed", {
          error: error?.message || error,
        });
      }
    }
