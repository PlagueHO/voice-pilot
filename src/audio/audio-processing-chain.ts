...
import { AudioMetrics, AudioProcessingChain, AudioProcessingConfig, AudioProcessingGraph } from '../types/audio-capture';
import { calculatePeak, calculateRms, computeBufferHealth, createEmptyMetrics, estimateSnr, mergeMetrics } from './audio-metrics';
...
export class WebAudioProcessingChain implements AudioProcessingChain {
    ...
    private readonly metricsState = new WeakMap<AudioProcessingGraph, MetricsState>();

    constructor(logger?: Logger) {
        this.logger = logger || new Logger('WebAudioProcessingChain');
    }

    async createProcessingGraph(stream: MediaStream, config: AudioProcessingConfig): Promise<AudioProcessingGraph> {
        try {
            const latencyHint: AudioContextLatencyCategory | number = typeof config.analysisIntervalMs === 'number'
            ...
            this.logger.error('Failed to create audio processing graph', { error: error?.message });
            throw error;
        }
    }

    async updateProcessingParameters(graph: AudioProcessingGraph, config: Partial<AudioProcessingConfig>): Promise<void> {
    ...
    disposeGraph(graph: AudioProcessingGraph): void {
        try {
            ...
            this.logger.warn('Failed to fully disconnect processing graph', { error: error?.message });
        }
    ...
    private applyProcessingLevels(gainNode: GainNode, config: Partial<AudioProcessingConfig>): void {
        if (config.autoGainControlLevel) {
            ...
            gainNode.gain.value = levelMap[config.autoGainControlLevel] ?? gainNode.gain.value;
        }
    }
}
