...
describe('SessionManagerImpl - Comprehensive Tests', () => {
    ...
    const createMockKeyService = () => ({
        ...
        onKeyRenewed: (handler) => ({ dispose: () => { } }),
        onKeyExpired: (handler) => ({ dispose: () => { } }),
        onAuthenticationError: (handler) => ({ dispose: () => { } })
    });
    const createMockTimerManager = () => ({
        startRenewalTimer: (sessionId, renewAtMs) => { },
        startTimeoutTimer: (sessionId, timeoutMs) => { },
        startHeartbeatTimer: (sessionId, intervalMs) => { },
        ...
        getNextScheduledEvent: (sessionId) => ({
            type: 'renewal',
            sessionId,
            scheduledAt: new Date(),
            timeRemainingMs: 300000
    ...
    beforeEach(() => {
        ...
        sessionManager = new session_manager_1.SessionManagerImpl(mockKeyService, mockTimerManager, mockConfigManager, mockLogger);
    });
    describe('Initialization and Lifecycle', () => {
        ...
        it('should throw if key service is not initialized', async () => {
            ...
            await assert.rejects(sessionManager.initialize(), /EphemeralKeyService must be initialized/);
        });
        it('should throw if config manager is not initialized', async () => {
            mockConfigManager.isInitialized = () => false;
            await assert.rejects(sessionManager.initialize(), /ConfigurationManager must be initialized/);
        });
        it('should dispose cleanly and end all active sessions', async () => {
            await sessionManager.initialize();
            const sessionInfo = await sessionManager.startSession();
            assert.strictEqual(sessionInfo.state, session_1.SessionState.Active);
    ...
    describe('Session Lifecycle Operations', () => {
        ...
        it('should enforce concurrent session limits', async () => {
            ...
            await assert.rejects(sessionManager.startSession(), /Maximum concurrent sessions.*exceeded/);
        });
        ...
        it('should throw error on session start if key service fails', async () => {
            ...
            await assert.rejects(sessionManager.startSession(), /Failed to obtain session credentials/);
        });
    });
    describe('Session State Management', () => {
        ...
        it('should retrieve session info correctly', () => {
            ...
            assert.strictEqual(retrieved.state, session_1.SessionState.Active);
        });
        ...
        it('should throw error when updating config for non-existent session', async () => {
            await assert.rejects(sessionManager.updateSessionConfig('non-existent', {}), /Session.*not found/);
        });
    ...
    describe('Session Renewal', () => {
        ...
        it('should handle renewal failure', async () => {
            ...
            const updated = sessionManager.getSessionInfo(sessionInfo.sessionId);
            assert.strictEqual(updated?.state, session_1.SessionState.Failed);
    ...
    describe('Event System', () => {
        ...
        it('should emit session started event', async () => {
            const disposable = sessionManager.onSessionStarted(async (event) => {
                eventCaptured = event;
            });
            sessionInfo = await sessionManager.startSession();
        ...
        it('should emit session ended event', async () => {
            sessionInfo = await sessionManager.startSession();
            const disposable = sessionManager.onSessionEnded(async (event) => {
                eventCaptured = event;
            });
            await sessionManager.endSession(sessionInfo.sessionId);
            assert.ok(eventCaptured);
        ...
        it('should emit session renewal events', async () => {
            sessionInfo = await sessionManager.startSession();
            let renewalEvents = [];
            const disposable = sessionManager.onSessionRenewed(async (event) => {
        ...
        it('should emit session error events', async () => {
            sessionInfo = await sessionManager.startSession();
            const disposable = sessionManager.onSessionError(async (event) => {
                eventCaptured = event;
            });
    ...
    describe('Diagnostics and Health Monitoring', () => {
        ...
        it('should provide session diagnostics', () => {
            ...
            assert.strictEqual(diagnostics.state, session_1.SessionState.Active);
        ...
        it('should throw error for non-existent session diagnostics', () => {
            assert.throws(() => sessionManager.getSessionDiagnostics('non-existent'), /Session.*not found/);
        });
        ...
        it('should throw error for non-existent session health check', async () => {
            await assert.rejects(sessionManager.testSessionHealth('non-existent'), /Session.*not found/);
        });
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should throw error if not initialized', async () => {
            const uninitializedManager = new session_manager_1.SessionManagerImpl();
            await assert.rejects(uninitializedManager.startSession(), /SessionManager not initialized/);
        });
        it('should handle timer callback errors gracefully', async () => {
            ...
            await sessionManager.handleRenewalRequired(sessionInfo.sessionId);
...
//# sourceMappingURL=session-manager-integration.test.js.map